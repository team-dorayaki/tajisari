...
function PropertyDeleteDialog({ count, isDeleting, error, onCancel, onConfirm }: PropertyDeleteDialogProps) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/45 px-5" role="presentation">
      ...
      >
        <span className="grid size-10 place-items-center rounded-full bg-[#fff0ed] text-[#e45f4c]">
          <TriangleAlert aria-hidden="true" className="size-5" />
        </span>
        <h2 id="property-delete-title" className="mt-4 text-lg font-bold">매물을 삭제할까요?</h2>
        <p id="property-delete-description" className="mt-2 text-sm leading-5 text-[var(--text-secondary)]">
          선택한 매물 {count}개가 삭제돼요. 삭제한 매물은 다시 복구할 수 없어요.
        </p>
        {error && <p className="mt-3 text-xs font-semibold text-[#e45f4c]" role="alert">{error}</p>}
